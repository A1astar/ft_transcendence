...
export async function apiGameManagement(
  fastify: FastifyInstance,
  games: Map<string, Game>,
  gameConnections: Map<string, Set<any>>
) {
...
export async function apiGameControl(
  fastify: FastifyInstance,
  games: Map<string, Game>,
  gameConnections: Map<string, Set<any>>
) {
  ...
  fastify.get(
    "/api/game-engine/cli/:gameId/move/:action",
    async (request, reply) => {
      const { gameId, action } = request.params as {
        gameId: string;
        action: string;
      };
      const game = games.get(gameId);
