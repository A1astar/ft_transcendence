...
function resolvePlayerSide(game: Game): "left" | "right" | "up" | "down" | "" {
    if ((game.mode === "remote2" || game.mode === "remote4") && game.players.left === 0) {
        ...
        (game.mode === "remote2" || game.mode === "remote4") &&
        game.players.left !== 0 &&
        game.players.right === 0
    ) {
        ...
        try {
            ...
            if (type === "keyPress") {
                if (game.mode === "remote2") {
                    if (key == "w" && playerside === "left") game.paddleMovement.leftUp = true;
                    ...
                    if (key == "w" && playerside === "left") game.paddleMovement.leftUp = true;
                        ...
                        game.paddleMovement.rightDown = true;
                    else if (key == "w" && playerside === "up") game.paddleMovement.upLeft = true;
                    else if (key == "s" && playerside === "up") game.paddleMovement.upRight = true;
                    ...
                    if (key == "w") game.paddleMovement.leftUp = true;
                    else if (key == "s") game.paddleMovement.leftDown = true;
                    else if (key == "p") game.paddleMovement.rightUp = true;
                    else if (key == "l") game.paddleMovement.rightDown = true;
                }
            } else if (type === "keyRelease") {
                if (game.mode === "remote2") {
                    if (key == "w" && playerside === "left") game.paddleMovement.leftUp = false;
                    else if (key == "s" && playerside === "left")
                    ...
                    if (key == "w" && playerside === "left") game.paddleMovement.leftUp = false;
                        ...
                        game.paddleMovement.rightDown = false;
                    else if (key == "w" && playerside === "up") game.paddleMovement.upLeft = false;
                    else if (key == "s" && playerside === "up") game.paddleMovement.upRight = false;
                    else if (key == "w" && playerside === "down")
                        game.paddleMovement.downRight = false;
                    else if (key == "s" && playerside === "down")
                        game.paddleMovement.downLeft = false;
                } else {
                    if (key == "w") game.paddleMovement.leftUp = false;
                    else if (key == "s") game.paddleMovement.leftDown = false;
                    else if (key == "p") game.paddleMovement.rightUp = false;
                    else if (key == "l") game.paddleMovement.rightDown = false;
                }
            }
        } catch (error) {
...
export async function handleWebSocket(
    ...
    gameConnections: Map<string, Set<any>>,
) {
        ...
        fastify.get("/api/game-engine/:gameId", {websocket: true}, (connection, req) => {
            ...
            try {
                try {
                    console.log(
                        chalk.yellow(
                            `WS connect URL: ${JSON.stringify(
                                (req as any).url ?? (req as any).raw?.url ?? "<unknown>",
                            )}`,
                        ),
                    );
                    console.log(
                        chalk.yellow(
                            `WS connect parsed query: ${JSON.stringify((req as any).query ?? {})}`,
                        ),
                    );
                } catch (e) {}
            ...
            function endGameOnLeave() {
                ...
                if (game.mode === "remote2") {
                    function getAliasForSide(side: "left" | "right") {
                        try {
                            if (
                                game &&
                                (game as any).playerAliases &&
                                (game as any).playerAliases[side]
                            ) {
                                return (game as any).playerAliases[side];
                            }
                        } catch (e) {}
                        if (side === "left") return (game as any).aliasLeft || "left";
                        return (game as any).aliasRight || "right";
                    }
                    let winner = "";
                    if (playerside === "left") {
                        winner = getAliasForSide("right");
                    } else if (playerside === "right") {
                        winner = getAliasForSide("left");
                    } else {
                        winner = "adversaire";
                    }
