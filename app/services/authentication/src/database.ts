...
export class SQLiteDatabase {
    ...
    try {
      ...
      try {
        ...
        if (error && error.code === "SQLITE_CONSTRAINT") {
          reply
            .code(409)
            .send({ error: "User with this name or email already exists" });
          return;
        }
  ...
  async loginUser(request: FastifyRequest, reply: FastifyReply): Promise<void> {
    ...
    try {
      ...
      console.log(
        "[auth] loginUser - attempting login for user:",
        validatedData.name
      );
      ...
      // Successful login
      console.log(
        chalk.green(`[auth] Login successful for user: ${validatedData.name}`)
      );
  ...
  async loginOrRegisterOAuthUser(
    ...
    if (!user) {
      ...
      let suffix = 1;
      while (true) {
        const nameCheck = this.sqlite
          .prepare("SELECT 1 FROM users WHERE name = ?")
          .get(name);
        if (!nameCheck) break;
