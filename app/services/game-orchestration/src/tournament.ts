...
export async function tournamentMatch(fastify: FastifyInstance) {
  ...
  fastify.post(
    "/api/game-orchestration/tournament/match-ended",
    async (request, reply) => {
      const { tournamentId, matchId } = request.body as {
        tournamentId: string;
        matchId: string;
      };
  ...
  fastify.post(
    "/api/game-orchestration/tournament/start-match",
    async (request, reply) => {
      ...
      if (targetMatch.status !== "waiting") {
        return reply.status(400).send({
          error: `Match is not in waiting status. Current status: ${targetMatch.status}`,
        });
      }
