...
export class ProfileViewBinder implements ViewEventBinder {
  bind() {
    ...
    document
      ...
      ?.addEventListener("click", this.onEnable2faClick);
    document
      .getElementById("disable2fa")
      ?.addEventListener("click", this.onDisable2faClick);
    console.log("ProfileViewBinder: bind called");
  }
  unbind() {
    document
      .getElementById("logout")
      ?.removeEventListener("click", this.onLogoutClick);
    document
      .getElementById("enable2fa")
      ?.removeEventListener("click", this.onEnable2faClick);
    document
      .getElementById("disable2fa")
      ?.removeEventListener("click", this.onDisable2faClick);
  }
  ...
  private onEnable2faClick = async (e: Event) => {
    ...
    try {
      ...
      if (data?.otpauthUrl) {
        ...
        alert(
          "2FA has been enabled. Please use your authenticator app to scan the QR code on this page."
        );
      } else {
