...
export class Tournament8LobbyViewBinder implements ViewEventBinder {
  bind() {
    document
      .getElementById("startTournament")
  ...
  private handleTournament8 = async () => {
    ...
    try {
      ...
      const matchPromises = playerNames.map(async (playerName) => {
        ...
        if (!res.ok)
          throw new Error(`Match request failed for player ${playerName}`);
        return res.json();
      });
      ...
      const tournament = results.find(
        (result) => result.status === "game created"
      ).match;
