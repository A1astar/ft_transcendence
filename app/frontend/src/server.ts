...
function readCertPair(): { key: Buffer; cert: Buffer } {
  ...
  throw new Error(
    "No TLS certs found in certs/. Expected server.key/server.crt or self.key/self.crt"
  );
}
...
function proxyWebSocket(req: http.IncomingMessage, socket: Duplex): void {
  ...
  if (urlObj.pathname.startsWith("/api/game-engine/")) {
    ...
    backendReq.on("upgrade", (backendRes, backendSocket) => {
