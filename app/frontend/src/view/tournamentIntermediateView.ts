...
export function renderTournamentIntermediate(
  info: TournamentIntermediateInfo,
  onNextGame: () => void
) {
  if (!appDiv) return;
  ...
  const winnerTitle = createSubheadingText(
    info.currentRound === 0 ? `${info.winner}` : `${info.winner} WINS!`
  );
  ...
  progressDiv.className =
    "mb-8 p-6 bg-gray-800/40 rounded-lg border border-amber-700/30";
  ...
  if (
    info.nextPlayers &&
    info.nextPlayers.length > 0 &&
    info.currentRound < info.totalRounds
  ) {
    const nextPlayersDiv = document.createElement("div");
    nextPlayersDiv.className =
      "mt-4 p-4 bg-slate-700/40 rounded-lg border border-slate-500/30";

    const nextPlayersTitle = document.createElement("h3");
    nextPlayersTitle.className = "text-lg font-bold text-slate-300 mb-2";
    nextPlayersTitle.textContent =
      info.currentRound === 0 ? "🎮 First Match:" : "⚔️ Next Match:";
    ...
    player1.className =
      "text-white font-semibold text-lg px-3 py-1 bg-indigo-500 rounded";
    ...
    player2.className =
      "text-white font-semibold text-lg px-3 py-1 bg-purple-500 rounded";
  ...
  if (info.currentRound === info.totalRounds) {
    ...
    backButton.className +=
      " text-lg px-8 py-3 bg-green-600 hover:bg-green-700 text-center";
    buttonContainer.appendChild(backButton);
  } else if (info.nextMatch) {
    // Next game ready
    const buttonText =
      info.currentRound === 0 ? "Start First Game" : "Start Next Game";
    const nextGameButton = createButtonForm(buttonText, "nextGame");
    nextGameButton.className +=
      " text-base px-8 py-3 bg-blue-600 hover:bg-blue-700 text-center";
    ...
    // Exit button
    const exitButton = createButtonLink(
      "/gameMenu",
      "Exit Tournament",
      "center"
    );
    exitButton.className +=
      " text-base px-4 py-2 bg-red-600 hover:bg-red-700 ml-4 text-center";
