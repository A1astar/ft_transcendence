...
export async function renderLogin() {
  if (appDiv) {
    ...
    if (user && user.username) {
      ...
      alreadyBox.appendChild(
        createSubheadingText(`Welcome back, ${user.username}`)
      );
      const nameDisplay = document.createElement("div");
      nameDisplay.className = "text-center my-4";
      const nameH3 = document.createElement("h3");
      nameH3.className =
        "text-3xl font-extrabold text-amber-300 drop-shadow-md font-lora";
    ...
    formDiv1.appendChild(
      createInputElement("text", "username", "Username/email")
    );
    formDiv1.appendChild(
      createInputElement("password", "password", "Password")
    );
    // Optional 2FA code input (used only if the account has 2FA enabled)
    formDiv1.appendChild(
      createInputElement("text", "twoFactorCode", "2FA Code (if enabled)")
    );
    ...
    loginBoxDiv.appendChild(
      createSubheadingText("Join the realm of shadows with account")
    );
    loginBoxDiv.appendChild(formDiv1);

    const oauth2BoxDiv = createBoxDiv("oauth2BoxDiv");
    oauth2BoxDiv.appendChild(
      createSubheadingText("Join the realm of shadows with Oauth2")
    );
