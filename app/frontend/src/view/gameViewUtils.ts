...
export function update3DMeshPos(
  meshElement: any,
  xPos: number,
  yPos: number,
  zPos: number
...
export function createPaddle(scene: any) {
  ...
  const tower1 = BABYLON.MeshBuilder.CreateCylinder(
    "tower",
    { diameter: 0.6 },
    scene
  );
  ...
  const tower2 = BABYLON.MeshBuilder.CreateCylinder(
    "tower",
    { diameter: 0.6 },
    scene
  );
...
export function createScoreBox2(scene: any) {
  ...
  scoreText.textHorizontalAlignment =
    BABYLON.GUI.Control.HORIZONTAL_ALIGNMENT_CENTER;
  scoreText.textVerticalAlignment =
