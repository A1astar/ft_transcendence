...
export function renderGameMenu() {
  if (appDiv) {
    ...
    RemoteGameformDiv.appendChild(
      createButtonForm("Remote 2 players", "Remote2")
    );
    RemoteGameformDiv.appendChild(
      createButtonForm("Remote 4 players", "Remote4")
    );

    const TournamentGameformDiv = createFormElement("tournamentGameForm");
    TournamentGameformDiv.appendChild(
      createButtonForm("Tournament 4 players", "Tournament4")
    );
    TournamentGameformDiv.appendChild(
      createButtonForm("Tournament 8 players", "Tournament8")
    );
    ...
    tournamentBox.appendChild(
      createSubheadingText("Tournament for Middle-earth")
    );
    tournamentBox.appendChild(TournamentGameformDiv);

    appDiv.appendChild(
      createVideoBackgroundDiv("../public/backgrounds/Sauron.mp4")
    );
    appDiv.appendChild(
