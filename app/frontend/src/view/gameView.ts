...
function setupScene4(canvas: HTMLCanvasElement) {
    ...
    const upP = createPaddle(scene);
    const downP = createPaddle(scene);
    ...
    return {engine, scene, ball, leftP, rightP, upP, downP, visionCone, scoreText};
}
...
function setupScene2(canvas: HTMLCanvasElement) {
    const engine = new BABYLON.Engine(canvas, true);
    const scene = new BABYLON.Scene(engine);
...
export function renderGame(
    ...
    let engine, scene, ball, leftP, rightP, upP, downP, visionCone, scoreText;

    if (nbPlayers === 2) {
        ({engine, scene, ball, leftP, rightP, visionCone, scoreText} = setupScene2(canvas));
        setupWebSocket2(matchInfos, ball, leftP, rightP, scoreText, onGameEnd);
    } else {
        ({engine, scene, ball, leftP, rightP, upP, downP, visionCone, scoreText} =
            setupScene4(canvas));
        setupWebSocket4(matchInfos, ball, leftP, rightP, scoreText, onGameEnd);
    }
